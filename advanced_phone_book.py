...
keys = keys = phonebook[0].keys()
#makes all the keys of the first item in the dictionary into a list called keys
#list of the "fieldnames" or names of the columns
#comma separated values
#allows table structured format
import csv

with open('phonebook.csv', 'w') as save:
    w = csv.DictWriter(save, keys)
    #brings in a write version of my csv file and uses the list of keys I made above
    w.writeheader()
    for contact in phonebook:
        w.writerow(contact)
        #pulls in the values for each dictionary
